pipeline {
    ...
    environment {
        ...
        SCAN_TOKEN = credentials('reactjs-scan-token')
    }
    ...
    stages {
        ...
        stage("Git") {
            steps {
                git branch: 'feature/devops_sagar', credentialsId: 'git_token_cs', url: 'https://github.com/v2dev/v2solutions-reactjs-boilerplate'
            }
        }
        ...
        //             def projectKey = "SaaS-Boilerplate-Reactjs"
        ...
        stage("Build Reactjs App") {
            steps {
                ...
                bat '@echo off'
                bat 'echo %WORKSPACE%'
                bat "echo 'Building reactjs application'"
                bat 'npm run build'
            }
        }

        // Copy built Reactjs code to S3 bucket
        stage("Copy Artifacts to S3") {
            steps {
                script {
                    def bucketName = "v2-reactjs-boilerplate"
